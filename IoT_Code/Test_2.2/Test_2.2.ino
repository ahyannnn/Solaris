...
unsigned long logInterval = 900000;      // 15 min default
unsigned long uploadInterval = 7200000;  // 2 hours default
...
void handleRoot() {
  server.send(200, "text/html",
    ...
    "Logging Interval (minutes):<br><input name='log'><br><br>"
    "Upload Interval (minutes):<br><input name='upload'><br><br>"
...
void handleSave() {
  ...
  if (logMin <= 0) logMin = 15;
  logInterval = (unsigned long)logMin * 60000;

  int uploadMin = server.arg("upload").toInt();
  if (uploadMin <= 0) uploadMin = 120;
  uploadInterval = (unsigned long)uploadMin * 60000;
...
void setup() {
  ...
  logInterval = (unsigned long)preferences.getUInt("log", 15) * 60000;
  uploadInterval = (unsigned long)preferences.getUInt("upload", 120) * 60000;
