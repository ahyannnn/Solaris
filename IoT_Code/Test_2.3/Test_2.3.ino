...
// ------------------- INTERVALS -------------------
#define DEFAULT_LOG_MIN 15
#define DEFAULT_UPLOAD_MIN 120
RTC_DATA_ATTR unsigned long logInterval = DEFAULT_LOG_MIN * 60 * 1000UL;      // millis
RTC_DATA_ATTR unsigned long uploadInterval = DEFAULT_UPLOAD_MIN * 60 * 1000UL;  // millis
RTC_DATA_ATTR unsigned long lastUploadTime = 0; // millis counter for uploads
...
void sendToServer(String timestamp, float lat, float lon, float irradiance) {
  ...
  if (code > 0) Serial.printf("🌐 Sent OK: %d\n", code);
...
void startAccessPoint() {
  ...
  server.on("/", []() {
    server.send(200, "text/html",
      ...
      "Logging Interval (min):<br><input name='log'><br><br>"
      "Upload Interval (min):<br><input name='upload'><br><br>"
  ...
  server.on("/save", []() {
    ...
    int logMin = server.arg("log").toInt();
    if (logMin <= 0) logMin = DEFAULT_LOG_MIN;
    logInterval = (unsigned long)logMin * 60 * 1000UL;

    int uploadMin = server.arg("upload").toInt();
    if (uploadMin <= 0) uploadMin = DEFAULT_UPLOAD_MIN;
    uploadInterval = (unsigned long)uploadMin * 60 * 1000UL;

    String newURL = server.arg("url");
    if (newURL.length() > 5) serverURL = newURL;

    preferences.begin("config", false);
    preferences.putString("ssid", savedSSID);
    preferences.putString("pass", savedPASS);
    preferences.putUInt("log", logMin);
    preferences.putUInt("upload", uploadMin);
...
void setup() {
  ...
  logInterval = (unsigned long)preferences.getUInt("log", DEFAULT_LOG_MIN) * 60 * 1000UL;
  uploadInterval = (unsigned long)preferences.getUInt("upload", DEFAULT_UPLOAD_MIN) * 60 * 1000UL;
  serverURL = preferences.getString("url", serverURL);
  preferences.end();

  if (!connectToWiFi(savedSSID, savedPASS)) startAccessPoint();
...
void loop() {
  ...
  // PYRANOMETER
  uint8_t result; float irradiance=-1;
  ...
  if (millis() - lastUploadTime >= uploadInterval) {
    ...
    sendToServer(timestamp, lat, lon, irradiance);
    lastUploadTime = millis();
  }

  // SLEEP for logInterval (15 min default)
  Serial.println("💤 Going to sleep...");
  esp_sleep_enable_timer_wakeup(logInterval * 1000 / 1000); // ESP expects microseconds
