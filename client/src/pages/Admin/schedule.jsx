...
const AdminSchedule = () => {
  ...
  // EFFECTS
  // ============================================================
  // Real-time table updates (no page refresh): refetch current tab page +
  ...
  // Charts + KPIs per tab from lightweight stats endpoints (no row dumps).
  ...
  const fetchPreAssessmentSchedules = async () => {
    try {
      ...
      // Server-side paging: ONE page of 10 rows (enrichment done by the API)
  ...
  const fetchInstallationProjects = async () => {
    try {
      ...
      // Server-side paging: ONE page of 10 installation projects
  ...
  // NOTE: filtering + slicing now happen server-side (page/limit/search params).
  // The pager below consumes backend total/totalPages directly.
                ...
                {statusChartData.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={statusChartData} layout="vertical" margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                      ...
                      <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-secondary, #667085)', fontSize: 11, fontWeight: 500 }} width={105} />
              ...
              <div className="schedule-chart-wrapper">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={monthlyChartData} margin={{ top: 10, right: 10, left: 5, bottom: 0 }}>
                    <defs>
                      <linearGradient id="colorScheduledSch" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#F39C12" stopOpacity={0.3} />
                        <stop offset="95%" stopColor="#F39C12" stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="colorCompletedSch" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="scheduled" name="Scheduled" stroke="#F39C12" strokeWidth={2.5} fillOpacity={1} fill="url(#colorScheduledSch)" dot={{ r: 3, fill: '#F39C12', strokeWidth: 2, stroke: 'var(--bg-card, #FFFFFF)' }} activeDot={{ r: 5 }} />
                    <Area type="monotone" dataKey="completed" name="Completed" stroke="#10B981" strokeWidth={2.5} fillOpacity={1} fill="url(#colorCompletedSch)" dot={{ r: 3, fill: '#10B981', strokeWidth: 2, stroke: 'var(--bg-card, #FFFFFF)' }} activeDot={{ r: 5 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
