...
// ---------------------------------------------------------------------------
// Standardized report header metadata (presentation only — no data changes)
// Header spec:
//   [Company Logo] [Company Name]
//   REPORT TITLE / TYPE OF REPORT
//   Generated by: [user] / Generated at: [time only] / Report Date: [range]
// ---------------------------------------------------------------------------
const resolveGeneratedBy = async (req, override) => {
  if (override && String(override).trim()) {
    return String(override).trim().slice(0, 80);
  }
  try {
    if (req?.user?.id) {
      const u = await User.findById(req.user.id).select('fullName email').lean();
      if (u?.fullName && String(u.fullName).trim()) return String(u.fullName).trim();
      if (u?.email) return u.email;
    }
    if (req?.user?.email) return req.user.email;
  } catch (err) {
    console.warn('⚠️ Could not resolve report author:', err.message);
  }
  return 'Administrator';
};

const formatTimeOnly = (date = new Date()) => {
...
// "Mar 1, 2026 - Mar 31, 2026" or "All dates" — used for the Report Date line.
const formatReportDateRange = (startDate, endDate) => {
  const s = formatDateOnly(startDate);
  const e = formatDateOnly(endDate);
...
const buildReportMeta = async (req, { reportTitle, dateRange, recordCount, generatedByOverride }) => {
  const generatedAt = new Date();
  const generatedBy = await resolveGeneratedBy(req, generatedByOverride);
  return {
    reportTitle,
    generatedBy,
    generatedAt,
    generatedAtTime: formatTimeOnly(generatedAt),
    reportDate: formatReportDateRange(dateRange?.startDate, dateRange?.endDate),
    recordCount: recordCount || 0
  };
};

// ✅ Standardized PDF header: branding row, title, then scannable metadata.
const addCompanyHeaderToPDF = (doc, meta = null) => {
  ...
  // --- Branding row: [Logo] [Company Name + address + tagline] (left aligned)
  const logoBuffer = getLogoBuffer();
  let brandX = left;
  if (logoBuffer) {
    try {
      doc.image(logoBuffer, left, 22, { fit: [logoSize, logoSize] });
      brandX = left + logoSize + logoGap;
    } catch (err) {
      console.warn('⚠️ Failed to add logo to PDF:', err.message);
    }
  }
  const brandWidth = width - (brandX - left);

  doc.fillColor(REPORT_THEME.navy).font('Helvetica-Bold').fontSize(17)
    .text(COMPANY_INFO.name, brandX, 26, { width: brandWidth, align: 'left', lineBreak: false });
  doc.font('Helvetica').fontSize(8).fillColor(REPORT_THEME.muted)
    .text(COMPANY_INFO.address, brandX, 47, { width: brandWidth, align: 'left', lineBreak: false });
  doc.font('Helvetica-Oblique').fontSize(8).fillColor(REPORT_THEME.blue)
    .text(COMPANY_INFO.tagline, brandX, 59, { width: brandWidth, align: 'left', lineBreak: false });

  // --- Brand divider
  doc.strokeColor(REPORT_THEME.blue).lineWidth(2)
    .moveTo(left, 84).lineTo(left + width, 84).stroke();

  // --- Report title + metadata (centered, clear hierarchy)
  if (meta?.reportTitle) {
    doc.fillColor(REPORT_THEME.navy).font('Helvetica-Bold').fontSize(17)
      .text(String(meta.reportTitle).toUpperCase(), left, 94, { width, align: 'center' });
    const metaY = doc.y + 4;
    doc.font('Helvetica').fontSize(8.5).fillColor(REPORT_THEME.muted);
    doc.text(`Generated by: ${meta.generatedBy}`, left, metaY, { width, align: 'center' });
    doc.text(`Generated at: ${meta.generatedAtTime}`, left, doc.y, { width, align: 'center' });
    doc.text(`Report Date: ${meta.reportDate}   •   ${meta.recordCount} record(s)`, left, doc.y, { width, align: 'center' });
    // Thin separator between metadata and content
    doc.strokeColor(REPORT_THEME.border).lineWidth(0.5)
      .moveTo(left, doc.y + 8).lineTo(left + width, doc.y + 8).stroke();
    doc.y += 16;
  } else {
    doc.y = 96;
  }
  ...
  doc.restore();
  if (meta?.reportTitle) doc._reportMeta = meta;
  else if (doc._reportMeta) {
    // Repeating header on a new table page keeps the same title/meta.
  }
};

const addPdfFooter = (doc) => {
  const meta = doc._reportMeta;
  const y = doc.page.height - 55;
  doc.save();
  doc.strokeColor(REPORT_THEME.border).lineWidth(0.5)
    .moveTo(50, y - 8).lineTo(doc.page.width - 50, y - 8).stroke();
  // Line-only footer when flagged (Services report) — no text beside the line.
  if (!doc._hideFooterText) {
    doc.fillColor(REPORT_THEME.muted).font('Helvetica').fontSize(7)
      .text(
        meta?.reportTitle
          ? `${COMPANY_INFO.name}  •  ${meta.reportTitle}  •  Generated by ${meta.generatedBy}`
...
const drawPdfTable = (doc, headers, rows, columnWidths) => {
  ...
  rows.forEach((row, rowIndex) => {
    if (y + rowHeight > doc.page.height - 58) {
      addPdfFooter(doc);
      doc.addPage();
      addCompanyHeaderToPDF(doc, doc._reportMeta || null);
      y = doc.y;
      drawHeader(y);
      y += rowHeight;
    }
...
// ✅ Standardized Excel header — same hierarchy as PDF/CSV (presentation only).
// Layout:
//   Row 1: [Company Name]   Row 2: address   Row 3: tagline   Row 4: divider
//   Row 5: REPORT TITLE      Row 6: Generated by / Generated at / Report Date
//   Row 8: section title (content heading)
const addCompanyHeaderToWorksheet = (
  ...
  const generatedBy = meta?.generatedBy || 'Administrator';
  const generatedAtTime = meta?.generatedAtTime || formatTimeOnly(new Date());
  const reportDate = meta?.reportDate || 'All dates';
  ...
  worksheet.mergeCells(`A10:${lastColumn}10`);
  ...
  worksheet.getCell("A5").value = String(title || '').toUpperCase();
  ...
  worksheet.getCell("A6").value =
    `Generated by: ${generatedBy}   •   Generated at: ${generatedAtTime}   •   Report Date: ${reportDate}   •   ${recordCount} record(s)`;
  ...
  worksheet.getCell("A10").value = sectionTitle;
...
// ✅ Shared Excel builder — standardized header + real logo + print footer.
const buildExcelReport = async ({
  ...
  // Print-friendly footer (does not alter data rows).
  const lastColumn = getExcelColumnName(worksheet.columns.length);
  const footerRowNumber = worksheet.rowCount + 2;
  worksheet.mergeCells(`A${footerRowNumber}:${lastColumn}${footerRowNumber}`);
  const footerCell = worksheet.getCell(`A${footerRowNumber}`);
  footerCell.value = meta?.generatedBy
    ? `${COMPANY_INFO.name}  •  ${reportTitle}  •  Generated by ${meta.generatedBy}`
    : `${COMPANY_INFO.name}  •  ${COMPANY_INFO.tagline}`;
  footerCell.font = { size: 9, italic: true, color: { argb: 'FF5E6B75' } };
  ...
  worksheet.headerFooter.oddFooter = `&C&9${COMPANY_INFO.name}  •  ${reportTitle}`;
...
const addLogoToExcel = async (worksheet, workbook) => {
  try {
    ...
    if (logoBuffer) {
      ...
      // Top-left of the branded header (rows 1-3); compact so it never covers text.
...
// Standardized CSV header — mirrors the PDF/Excel header (presentation only).
const buildCsvHeader = ({ reportTitle, meta }) => {
  const lines = [];
  lines.push(`${COMPANY_INFO.name}`);
  lines.push(`${COMPANY_INFO.address}`);
  lines.push(`${COMPANY_INFO.tagline}`);
  lines.push(`${String(reportTitle || '').toUpperCase()}`);
  lines.push(`Generated by: ${meta?.generatedBy || 'Administrator'}`);
  lines.push(`Generated at: ${meta?.generatedAtTime || formatTimeOnly(new Date())}`);
  lines.push(`Report Date: ${meta?.reportDate || 'All dates'}`);
  lines.push(`Records: ${meta?.recordCount ?? 0}`);
  lines.push(``);
  return lines.join('\n') + '\n';
};
...
exports.getSiteAssessmentReport = async (req, res) => {
  try {
    ...
    reportData.generatedBy = await resolveGeneratedBy(req, req.query.generatedBy);
    reportData.generatedAtTime = formatTimeOnly(reportData.generatedAt);
    reportData.reportDate = formatReportDateRange(reportData.dateRange?.startDate, reportData.dateRange?.endDate);
...
exports.exportSiteAssessmentReport = async (req, res) => {
  try {
    const { format, data, generatedBy: generatedByOverride } = req.body;
    ...
    const meta = await buildReportMeta(req, {
      reportTitle: 'Site Assessment Report',
      dateRange: data.dateRange,
      recordCount: data.assessments?.length || 0,
      generatedByOverride: generatedByOverride || data.generatedBy
    });

    if (format === 'csv') {
      let csvContent = buildCsvHeader({ reportTitle: 'Site Assessment Report', meta });
    ...
    if (format === 'pdf') {
      ...
      addCompanyHeaderToPDF(doc, meta);
...
exports.exportProjectSummaryReport = async (req, res) => {
  try {
    ...
    const meta = await buildReportMeta(req, {
      ...
      dateRange: data.dateRange,
      recordCount: data.projects?.length || 0,
      generatedByOverride: generatedByOverride || data.generatedBy
    });

    if (format === 'csv') {
      let csvContent = buildCsvHeader({ reportTitle: 'Project Summary Report', meta });
...
exports.exportFinancialReport = async (req, res) => {
  try {
    ...
    const meta = await buildReportMeta(req, {
      ...
      dateRange: data.dateRange,
      recordCount: data.payments?.length || 0,
      generatedByOverride: generatedByOverride || data.generatedBy
    });

    if (format === 'csv') {
      let csvContent = buildCsvHeader({ reportTitle: 'Financial Report', meta });
...
exports.exportClientTransactionReport = async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      let csvContent = buildCsvHeader({ reportTitle: 'Clients Report', meta });
...
// Helper function to get services data - ONLY 6 columns
const getServicesData = async (queryParams) => {
  const { startDate, endDate, serviceType, status } = queryParams;

  let query = {};
  if (serviceType && serviceType.trim()) {
    query.serviceType = serviceType.trim();
  }
  ...
  if (startDate && endDate) {
    ...
    const end = new Date(endDate);
    if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
      // Include the full calendar days (bare YYYY-MM-DD parses as midnight,
      // which would otherwise exclude everything created on the end date).
  ...
  const processedServices = requests.map(r => {
    const populatedName = `${r.clientId?.contactFirstName || ''} ${r.clientId?.contactLastName || ''}`.trim();
    const statusRaw = r.status || 'pending';
    const statusDisplay = statusRaw.charAt(0).toUpperCase() + statusRaw.slice(1);

    return {
      _id: r._id,
      reference: r.referenceNo || 'N/A',
      clientName: (r.fullName && r.fullName.trim()) || populatedName || 'N/A',
      clientContact: (r.phone && r.phone.trim()) || r.clientId?.contactNumber || 'N/A',
...
exports.getServicesReport = async (req, res) => {
  try {
    console.log('[Services] Fetching report with params:', req.query);
    const reportData = await getServicesData(req.query);
    reportData.generatedBy = await resolveGeneratedBy(req, req.query.generatedBy);
    reportData.generatedAtTime = formatTimeOnly(reportData.generatedAt);
    reportData.reportDate = formatReportDateRange(reportData.dateRange?.startDate, reportData.dateRange?.endDate);
    return res.json({ success: true, report: reportData });
...
// @desc    Export Services Report
// @route   POST /api/admin/reports/export (with type: services)
// @access  Private (Admin)
exports.exportServicesReport = async (req, res) => {
  try {
    const { format, data, generatedBy: generatedByOverride } = req.body;
    ...
    const meta = await buildReportMeta(req, {
      ...
      dateRange: data.dateRange,
      recordCount: data.services?.length || 0,
      generatedByOverride: generatedByOverride || data.generatedBy
    });

    if (format === 'csv') {
      let csvContent = buildCsvHeader({ reportTitle: 'Services Report', meta });

      // ✅ Only 6 columns
      const headers = [
        'Reference', 'Client Name', 'Contact', 'Service Type', 'Preferred Date', 'Status'
      ];
      csvContent += headers.join(',') + '\n';

      const rows = (data.services || []).map(s => [
        `"${(s.reference || 'N/A').replace(/"/g, '""')}"`,
        `"${(s.clientName || 'N/A').replace(/"/g, '""')}"`,
        `"${(s.clientContact || 'N/A').replace(/"/g, '""')}"`,
        `"${(s.serviceType || 'N/A').replace(/"/g, '""')}"`,
        s.preferredDate ? new Date(s.preferredDate).toLocaleDateString() : 'N/A',
        `"${(s.status || 'N/A').replace(/"/g, '""')}"`
      ]);

      csvContent += rows.map(row => row.join(',')).join('\n');

      res.setHeader('Content-Type', 'text/csv; charset=utf-8');
      res.setHeader('Content-Disposition', `attachment; filename=services-report-${Date.now()}.csv`);
      return res.send(csvContent);
    }
    ...
    if (format === 'pdf') {
      ...
      const doc = new PDFDocument({ margin: 50, size: 'A4' });
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=services-report-${Date.now()}.pdf`);
      doc.pipe(res);
      // Line-only footer for this report (no text beside the line).
      doc._hideFooterText = true;

      addCompanyHeaderToPDF(doc, meta);

      doc.fillColor(REPORT_THEME.navy).fontSize(11).font('Helvetica-Bold').text('Service Details');
      doc.moveDown(0.4);

      // ✅ Only 6 columns
      if (data.services && data.services.length > 0) {
        const headers_pdf = ['Reference', 'Client Name', 'Contact', 'Service Type', 'Preferred Date', 'Status'];
        const rows = data.services.map(s => [
          s.reference,
          s.clientName,
          s.clientContact,
        ...
        drawPdfTable(doc, headers_pdf, rows, [78, 96, 78, 118, 68, 62]);
      }
    ...
    if (format === 'xlsx') {
      ...
      await buildExcelReport({
        ...
        rows: (data.services || []).map(s => [
          ...
          s.serviceType,
          s.preferredDate
            ? new Date(s.preferredDate).toLocaleDateString()
            : "N/A",
          s.status
        ])
...
exports.generateCustomReport = async (req, res) => {
  try {
    ...
    if (!type) {
      return res.status(400).json({
        success: false,
        message: 'Missing required field: type. Options: site-assessment, project-summary, financial, client-transaction, services'
      });
    }
    ...
    if (type === 'site-assessment') {
      ...
      reportData = await getServicesData({
        startDate: dateRange?.startDate,
        endDate: dateRange?.endDate,
      ...
      return res.status(400).json({
        success: false,
        message: `Invalid report type '${type}'. Options: site-assessment, project-summary, financial, client-transaction, services`
      });
    }
...
exports.exportReport = async (req, res) => {
  try {
    ...
    if (type === 'site-assessment') {
      ...
      return exports.exportServicesReport(req, res);
    }
